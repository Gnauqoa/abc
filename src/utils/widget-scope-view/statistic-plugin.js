...
import { getRangeSelections } from "../widget-line-chart/selection-plugin";
import { max, mean, min, round, std } from "mathjs";
import { createHiddenDataLineId } from "../widget-line-chart/legend-plugin";
import {
  linearRegression,
  quadraticRegression,
  powerRegression,
...
const getDataStatistic = ({ datasets, boxRange, isDefaultXAxis, statisticOptionId, sensorInfo }) => {
  const result = [];
...
// ======================================= STATISTIC OPTION =======================================
export const addStatisticNote = ({ chartInstance, sensors, pageId, isDefaultXAxis, statisticOptionId, widgetId }) => {
  try {
    ...
    const sensorInfo = createSensorInfo(sensors[0]);
    ...
    for (const dataRunPreview of dataRunPreviews) {
      ...
      const statisticsResult = getDataStatistic({ datasets, boxRange, isDefaultXAxis, statisticOptionId, sensorInfo });
      if (statisticsResult.length === 0) continue;

      for (const statisticResult of statisticsResult) {
        const { sensorInfo, min, max, mean, std, regression, x1, x2 } = statisticResult;
