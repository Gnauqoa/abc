...
export const getChartAnnotationByDelta = (delta) => {
  ...
  const deltaX = {
    ...
    content: [`Δx=${(Math.round((Number(delta.xMax) - Number(delta.xMin)) * 100) / 100).toFixed(2)}`],
  ...
  const deltaY = {
    ...
    backgroundColor: DELTA_BOX_BACKGROUND,
    borderColor: DELTA_BOX_BORDER,
    content: [`Δy=${(Math.round((Number(delta.yMax) - Number(delta.yMin)) * 100) / 100).toFixed(2)}`],
